        ...
        const ordersWithDetails = await Promise.all(
          ordersData.map(async (order: any) => {
            const [restaurantResponse, userResponse] = await Promise.all([
              axios.get(`${serverDomain}/api/restaurants/${order.restaurantId}`),
              axios.get(`${serverDomain}/api/users/${order.customerId}`)
            ]);
                    ...
                    <ul className="list-disc list-inside">
                      {order.orderItems.map((item: any) => (
                        <li key={item.id} className="text-sm text-gray-500">
                          {`${item.menuItem.name} - Quantity: ${item.quantity} - Price: ${item.price} TND`}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <p className="text-sm text-gray-500 mt-2">{`Total: ${order.totalPrice} TND`}</p>
                  <p className="text-sm text-gray-500">{`Status: ${order.status}`}</p>
                  <p className="text-sm text-gray-500">{`Restaurant: ${order.restaurantName || 'N/A'}`}</p>
                  <p className="text-sm text-gray-500">{`Date: ${order.createdAt ? new Date(order.createdAt).toLocaleString() : 'N/A'}`}</p>
                  <p className="text-sm text-gray-500">{`User: ${order.userName || 'N/A'}`}</p>
                </div>
